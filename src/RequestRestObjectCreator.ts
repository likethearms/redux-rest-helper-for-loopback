...
import { LoopbackFilter, RequestRestObjectCreatorRequests } from "./interfaces";
...
export class RequestRestObjectCreator<T> {
  url: string;
  ...
  getAll(filter?: LoopbackFilter): Promise<T[]> {
    return this.getRequest(this.url, filter).then(
      this.handleArrayResponse.bind(this)
    );
  }

  getById(id: string | number, filter?: LoopbackFilter): Promise<T> {
    return this.getRequest(this.getUrlWithEnd(id), filter).then(
      this.handleResponse.bind(this)
    );
  }

  delete(id: string | number): Promise<T> {
    return Axios.delete(this.getUrlWithEnd(id)).then(
      this.handleResponse.bind(this)
    );
  }

  create(body: T): Promise<T> {
    return Axios.post(this.url, this.cleanRequestBody(body)).then(
      this.handleResponse.bind(this)
    );
  }
  ...
  update(id: string | number, body: T): Promise<T> {
    return Axios.patch(
      this.getUrlWithEnd(id),
      this.cleanRequestBody(body)
    ).then(this.handleResponse.bind(this));
  }

  count(filter?: LoopbackFilter): Promise<{ count: number }> {
    return this.getRequest(this.getUrlWithEnd("count"), filter, "where").then(
      (r: AxiosResponse) => this.response<{ count: number }>(r.data)
    );
  }
